<template lang="pug">
  div.demo-alignment
    vx-popup.width-popup-75( :title="'VENTANA DE BÚSQUEDA DE CUENTAS PROGREXA'" :active="popupAbrir" @close="cancelar" :click-not-close="false" classContent="popup-example")
      ...
      vx-table.style-vxtable(idRef="listarCuentaProgrexa" ref="containerTable" max-items="10" :data="listarCuentaProgrexa" search search-sticky search-popup pagination rowsPage :arrayRows='[25,50,100]' :no-data-text="`${$t('No se encontraron registros')}`")
        template(slot="thead")
          vx-th.uppercase(min-width="100" center ) {{$t('Acciones')}}
          vx-th.uppercase(min-width="200" sort-key="moneda_descripcion") {{$t('Moneda')}}
          vx-th.uppercase(min-width="200" sort-key="cuenta_fintech_numerocuenta") {{$t('Número de Cuenta')}}
          vx-th.uppercase(min-width="140" sort-key="cuenta_fintech_estado") {{$t('Estado')}}
        template(slot-scope="{data}")
          tbody(class="table-text-sm")
            vx-tr( :class="{'style-danger-vxtable' : !tr.cuenta_fintech_estado }" :data="tr" :key="indextr" v-for="(tr,indextr) in data")
              vx-td.text-center
              ...
              vx-td.text-left(:data="tr.cuenta_fintech_numerocuenta") {{tr.cuenta_fintech_numerocuenta}}
              vx-td.text-center(:data="tr.cuenta_fintech_estado")
                span(v-if="tr.cuenta_fintech_estado" class="justify-center") {{$t('ACTIVO')}}
...
export default {
  ...
  name: 'PopupFiltrarCuentaProgrexa',
  mixins: [request],
  ...
  methods:{
    ...
    enviarSeleccionados(){
      let value = '';
      let label = '';
      this.listarCuentaProgrexa.forEach((element) => { 
        if(element.marcado){
          value = value ? value+','+element.cuenta_fintech_id:value+element.cuenta_fintech_id;
          label = label ? label+', '+element.cuenta_fintech_numerocuenta:label+element.cuenta_fintech_numerocuenta;
        } 
      });
      this.$emit('selectedAccountProgrexa', {value: value, label: label});
      this.cancelar();
    },
    ...
    async listar() {
      try { 
        ...
        let params = "listar_busqueda_cuenta_fintech?";        
        ...
        if (response.estado) {
          ...
          this.listarCuentaProgrexa = response.data;
        } else {
          this.listarCuentaProgrexa = [];
        }
      } catch (error) {
