...
div#modificar-contrasena

  cabecera( :titulo="$t('Modificar Contraseña')"  perfil @cancelar='' @aceptar='' :btn-nuevo="false" :btn-cancelar="false" :btnExportar="false" :btnListarTodo="false" :btnFiltrar="false")
  ...
  methods: {
    ...
    async modificar() {
      ...
      try {
        ...
        let { data } = await this.requestApi( "modificar_contrasenia_backoffice", put, "PUT");
        return data;
      } catch (error) {
        throw new Error(error);
      } finally {
    ...
    async verificarToken(){
      try {
        this.$vs.loading({ type: "point" });
        let { data } = await this.requestApi(`verificar_token?token=${localStorage.getItem('token')}`);
