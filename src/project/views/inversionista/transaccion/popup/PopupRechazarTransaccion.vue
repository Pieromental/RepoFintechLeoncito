<template lang="pug">
  div.demo-alignment
    vx-popup.width_rechazar_transaccion( :title="$t('RECHAZAR TRANSACCIÓN')" :active="popupAbrir" @close="cancelar" :click-not-close="false" fullscreen classContent="popup-example" )
      div.vx-row
      ...
      div.vx-row
        div.vx-col.w-full.vs-justify-flex-end
            vs-button.font-label.py-2.style-button-seccion.mr-1(color="danger" size="small" @click="cancelar" type="filled")
  ...
  methods:{
    ...
    async validarFormulario(){      
      try {       
        ...
        let status = await swal.fire({ title: this.$t('Mensaje de Sistema'), text: mensaje, showCancelButton: true, confirmButtonText: this.$t('Si'), cancelButtonText: this.$t('No') });
        if (status.value){ 
          this.$vs.loading({ type: "point" });
          let put={};
          put.id = this.parametro;
          put.razon_rechazo = this.observacion;
          put.ip = await require('public-ip').v4(); 
          let { data } = await this.requestApi('rechazar_transaccion', put,"PUT");
...
<style >
.width_rechazar_transaccion .vs-popup {
...
.con-vs-popup.width_rechazar_transaccion .vs-popup--content{
  height: auto !important;
}
.con-vs-popup.fullscreen.width_rechazar_transaccion .vs-popup{
  height: auto !important;
}
