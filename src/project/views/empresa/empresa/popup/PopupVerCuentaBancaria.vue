<template lang="pug">
    div
      ...
      vx-popup.width_popup_cuenta_contable_formulario( :title="$t('VER CUENTA BANCARIA')" :active="popupRazonActive" @close="cancelar" :click-not-close="false" fullscreen classContent="popup-example" )
        div.vx-row
          div.vx-col.w-full(class="md:w-1/3")
            div.vx-row.mb-3
          ...
          div.vx-col.w-full(class="md:w-1/3")         
            ...
            div.vx-row.mb-3
              ...
              div.vx-col.w-full(class="sm:w-2/3")
                texts-box.w-full(size="small" v-model="dataCuenta.nombre" :disabled="true")
            div.vx-row.mb-3
              div.vx-col.w-full(class="sm:w-1/3")
                span.font-label.bold {{ $t("Tipo de Documento:") }} 
              div.vx-col.w-full(class="sm:w-2/3")
                texts-box.w-full(size="small" v-model="dataCuenta.tipo_documento" :disabled="true") 
            div.vx-row.mb-3
              div.vx-col.w-full(class="sm:w-1/3")
                span.font-label.bold {{ $t("Número de Documento:") }} 
              div.vx-col.w-full(class="sm:w-2/3")
                texts-box.w-full(size="small" v-model="dataCuenta.numero_documento" :disabled="true") 
            div.vx-row.mb-3
              div.vx-col.w-full(class="sm:w-1/3")
        ...
        div.vx-row.vs-justify-center(v-if="tipo=='revisar'")
          vx-button.mr-2(@click="aprobar(dataCuenta.id_cuentabancaria, dataCuenta.id_usuario, dataCuenta.codigo_estado_usuario)") {{$t('Aprobar')}}
...
export default {
  ...
  methods:{
    ...
    async aprobarFuncion(idAux, idUsuario){      
      try {    
        ...
        let { data } = await this.requestApi('aprobar_revision_cuenta_bancaria_bo', put,"PUT");
        return data;     
      } catch (error) {
    ...
    async aprobar(id, idUsuario, estado){
      try {
        if(estado == "ACT"){
          ...
          if (status.value) {
            let response = await this.aprobarFuncion(id, idUsuario);
