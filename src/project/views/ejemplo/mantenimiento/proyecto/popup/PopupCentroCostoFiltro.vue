<template lang="pug">
  //- div
  //-   div.demo-alignment
  //-     vx-popup.width_centro_costo_filtro( :title="'CENTRO DE RESPONSABILIDAD'" :active="popupCentroCostoActive" @close="closePopup" :click-not-close="false" fullscreen classContent="popup-example")
  ...
  //-       vx-table.style-vxtable( idRef="listaCentroCosto" ref="containerTable" max-items="10" search tab pagination  rowsPage :arrayRows='[25,50,100]' :data="listaCentroCosto" :no-data-text="$t('No se encontraron registros.')" )
  //-         template(slot="thead")
  //-           vx-th.uppercase(min-width="90" center) {{$t('Acciones')}}
  //-           vx-th.uppercase(min-width="100" sort-key="codigo") {{$t('Código')}}
  //-           vx-th.uppercase(min-width="160" sort-key="descripcion") {{$t('Descripción')}}
  //-           vx-th.uppercase(min-width="100" sort-key="administrativo") {{$t('Administrativo')}}
  //-           vx-th.uppercase(min-width="100" sort-key="finanzas") {{$t('Finanzas')}}
  //-           vx-th.uppercase(min-width="100" sort-key="ventas") {{$t('Ventas')}}
  //-           vx-th.uppercase(min-width="100" sort-key="produccion") {{$t('Producción')}}
  ...
  //-                 vx-td.text-left.bold(style="border-bottom: solid 1px" colspan="8" rowspan="1") {{tr.codigo}} - {{tr.descripcion}}
  ...
  //-                 vs-td.text-left(:data="tr.codigo") {{tr.codigo}}
  //-                 vs-td.text-left(:data="tr.descripcion") {{tr.descripcion}}  
  //-                 vx-td.text-center(:data="tr.administrativo")
  //-                   feather-icon.style-icon(:icon="tr.administrativo ? 'CheckIcon': 'XIcon'")
  //-                 vx-td.text-center(:data="tr.finanza")
  //-                   feather-icon.style-icon(:icon="tr.finanza ? 'CheckIcon': 'XIcon'")
  //-                 vx-td.text-center(:data="tr.venta")
  //-                   feather-icon.style-icon(:icon="tr.venta ? 'CheckIcon': 'XIcon'")
  //-                 vx-td.text-center(:data="tr.produccion")
  //-                   feather-icon.style-icon(:icon="tr.produccion ? 'CheckIcon': 'XIcon'")
...
//   data: () => ({
//     parentx: '#parentx-popup-centro-costo-filtro',
//     popupActive: false,
//     ocultarChip: false,
//     listaCentroCosto: [],
//     filtros: [
...
//     enviarSeleccionados() {
//       let cuentas = '';
//       this.listaCentroCosto.forEach((element) => { if(element.marcado) cuentas = cuentas ? cuentas+','+element.id:cuentas+element.id });
//       this.$emit('selectedCenter', cuentas);
//     },
...
//         let { data } = await this.requestApi(`listar_centro_costo_defecto_filtro?idCentroCosto=${obj.criterio0.codigo}&descripcion=${obj.criterio0.descripcion}&incluirInactivo=${obj.incluirInactivos == 'SI' ? true : false}&busquedaExacta=${obj.busquedaExacta == 'SI' ? true : false}`);
//         let response = data.data;
//         if (response.estado) {
//           response.data.forEach(element => {
//             element.marcado = false
//           });
//           this.listaCentroCosto = response.data;
//         } else {
//           this.listaCentroCosto = [];
//         }
//       } catch (error) {
