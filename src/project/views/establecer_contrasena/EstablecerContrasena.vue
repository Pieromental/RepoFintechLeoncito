<template lang="pug">
  ...
  div(class="h-screen flex")
    ...
    div.establecer-contrasena.p-10.mx-auto.my-auto

      div.mb-5(style="text-align: center;")

        span.bold-establecer-contrasena Establecer Contraseña
      ...
      div.w-full
        ...
        div.vx-row.pt-2.mt-2
          ...
          div.vx-col.w-full(class="sm:w-1/3")
            span.bold-establecer-contrasena.font-label Nueva Contraseña:
          ...
          div.vx-col.w-full(class="sm:w-2/3")
            div(style="position:relative;")
              ...
              feather-icon.style-icon.icon-password-establecer.p-0(v-if="showNewPassword" @click.stop="verNewPassword()" :title="$t('Mostrar')" icon='EyeIcon')
              feather-icon.style-icon.icon-password-establecer.p-0(v-else @click.stop="verNewPassword()" :title="$t('Ocultar')" icon='EyeOffIcon')
            div
        ...
        div.vx-row.pt-2.mt-2
          ...
          div.vx-col.w-full(class="sm:w-1/3")
            span.bold-establecer-contrasena.font-label Repetir Contraseña
          ...
          div.vx-col.w-full(class="sm:w-2/3")
            div(style="position:relative;")
              ...
              feather-icon.style-icon.icon-password-establecer.p-0(v-if="showConfirmPassword" @click.stop="verConfirmPassword()" :title="$t('Mostrar')" icon='EyeIcon')
              feather-icon.style-icon.icon-password-establecer.p-0(v-else @click.stop="verConfirmPassword()" :title="$t('Ocultar')" icon='EyeOffIcon')
            div
        ...
        div.vx-row.pt-3.mt-3.vs-justify-flex-end
          ...
          vs-button.font-label.btn-establecer-contrasena.py-2.px-3.mr-4(@click="enviar" size="small" color="primary") Aceptar
  ...
  methods: {
    ...
    async enviar() {
      try {
        ...
        if(status.value){
          let response = await this.establecer();
    ...
    async establecer() {
      ...
      try {
        ...
        let { data } = await this.requestApi( "establecer_clave_back_office", post, "POST");
        return data;
      } catch (error) {
        throw new Error(error);
      } finally {
...
.icon-password-establecer {
  position: absolute !important; 
  right: 0rem;
  top: 0.4rem;
}
.btn-establecer-contrasena{
  width: 5rem;
}
</style>