<template>
  <div class="vx-auto-suggest mx-1">
    <div class="flex items-center relative">
      ...
      <vs-input
        ...
        :class="inputClassses"
        class="z-50 color-navbar-search"
        icon-pack="feather"
        icon="icon-search"
        icon-no-border
        v-model="searchQuery"
    ...
    <ul
      ...
      tabindex="-1"
    >
      <li ref="grp_list" v-for="(suggestion_list, grp_name, grp_index) in filteredData" :key="grp_index" class="auto-suggest__suggestion-group-container">
        <!-- Group Header -->
  ...
  methods: {
    ...
    filter_grp(grp) {
      let exactEle = grp.data.filter((item) => {
        return this.slugify(item[grp.key]).startsWith(this.slugify(this.searchQuery.toLowerCase()));
      });

      let containEle = grp.data.filter((item) => {
        return (
          !this.slugify(item[grp.key]).startsWith(this.slugify(this.searchQuery.toLowerCase())) &&
          this.slugify(item[grp.key]).indexOf(this.slugify(this.searchQuery.toLowerCase())) > -1
        );
      });
