<template lang="html">
  <div :class="[`vs-tabs-${color}`, `vs-tabs-position-${position}`]" class="con-vs-tabs vs-tabs">
    <div class="con-ul-tabs font-label" :class="[{ 'header-tabs-fixed': fixed }, contentAreaClass]" :style="{ top: top }">
      <ul ref="ul" :class="[`ul-tabs-${alignment}`]" class="ul-tabs vs-tabs--ul pt-3">
        <li
          :key="index"
          ...
          class="vs-tabs--li vs-align-items"
    ...
    <div :class=" tabDefault ? 'con-slot-tabs-default' :'con-slot-tabs'" :style="{ marginTop: container }">
      <slot />
    </div>
  </div>
</template>
