...
export default {
  ...
  render(h) {
    ...
    let attr = {
      class: ['feather-icon select-none relative featherstyle'],
      on: {
    ...
    if (this.to) attr.attrs['href'] = `${process.env.VUE_APP_ENCRYPT_PUBLICPATH}${this.to}`;
    let children = [svg];
    if (this.badge) children.push(badgeEl);
    return h(element, attr, children);
  },
  methods: {
    clickFeather(event) {
