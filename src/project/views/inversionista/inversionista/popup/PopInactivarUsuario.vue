        ...
        vx-card.card-sticky-floating.styleSticky 
          ...
          div.vx-row
            div.vx-col.w-full
              div.vx-row.mb-3
                div.vx-col.w-full
                  span.bold.font-label {{$t('Observaciones de Inactivacion')}} 
                    span.font-label.bold.text-danger  *                    
                div.vx-col.w-full
                  vs-input.w-full(size="small" autocomplete="off" id ="observacion" ref="observacion" name="observacion"  v-model="observacion"  @focus="focus('observacion')"  v-mask="{ mask:funcionMascara(100,'D'), tokens:{ 'D': {pattern: /[A-Za-z0-9\\W\\w\\s]/}}}" @blur="validarObservacion();blur('observacion')" )
  ...
  methods:{
    ...
    async validarFormulario(){
      try {  
        ...
        let {data} = await this.requestApi('inactivar_registro_usuario_inversionista', put,"PUT");
