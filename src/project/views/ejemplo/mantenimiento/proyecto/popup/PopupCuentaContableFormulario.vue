<template lang="pug">
  //- div
  //-   div.demo-alignment
  //-     vx-popup.width_popup_cuenta_contable_formulario(:title="$t('Lista de Cuentas Contables')" :active='popupActive' @close="closePopup" :click-not-close="false" fullscreen classContent="popup-example")
  ...
  //-       div.mx-3
  //-         strong
  //-           small.text-danger {{ "** " + $t("Haga doble click para seleccionar una cuenta contable") }}
  ...
  //-         template(slot="thead")
  //-           vx-th.uppercase( min-width="120" sort-key="idCuentaContable") {{$t('Código')}}
  //-           vx-th.uppercase( min-width="150" sort-key="cuentaContable") {{$t('Descripción')}}
  //-           vx-th.uppercase( min-width="90" sort-key="idCuentaAnterior") {{$t('Cuenta Anterior')}}
  //-           vx-th.uppercase( min-width="90" sort-key="cuentaAnterior") {{$t('Descripción Anterior')}}
  ...
  //-               vx-td.text-left(:data="tr.idCuentaContable") {{tr.idCuentaContable}}
  //-               vx-td.text-left(:data="tr.cuentaContable") {{tr.cuentaContable}}
  //-               vx-td.text-center(:data="tr.idCuentaAnterior") {{tr.idCuentaAnterior}}
  //-               vx-td.text-left(:data="tr.cuentaAnterior") {{tr.cuentaAnterior}}
...
//   computed: {
//     listado() {
//       return this.listadoCuentaContable;
//     }
//   },
...
//       await this.listarCuentaContable();
//     },
...
//     async listarCuentaContable() {
//       try {
//         if(!this.company) return
//         this.$vs.loading({ type: 'point' });
//         let obj = helperFunctions.mapFilters(this.filtros);
//         let { data } = await this.requestApi( `listar_cuenta_contable_requerimiento?companiaSeleccionada=${this.company}&idCuentaContable=${obj.criterio0.codigo}&descripcion=${obj.criterio0.descripcion}&cuentaAnterior=${obj.criterio0.cuentaAnterior}&descripcionCuentaAnterior=${obj.criterio0.descripcionAnterior}&busquedaExacta=${obj.busquedaExacta == 'SI' ? true : false}&incluirInactivos=${obj.incluirInactivos == 'SI' ? true : false}`);
//         let response = data.data;
//         if (response.estado) {
//           this.listadoCuentaContable = response.data;
//         } else {
