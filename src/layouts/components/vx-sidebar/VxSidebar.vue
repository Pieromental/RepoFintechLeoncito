      ...
      <div @mouseenter="sidebarMouseEntered" @mouseleave="sidebarMouseLeave">
        <div class="header-sidebar flex items-end justify-between" slot="header">
          <div class="logo flex items-center">
            <img
              :src="reduce && !isMouseEnter ? logoIcono : logo"
              alt="logo"
              class="w-logo mr-4"
              :class="reduce && !isMouseEnter ? 'w-logo-reduce' : ''"
              v-if="reduce && !isMouseEnter ? logoIcono : logo"
            />
            <!-- <span
              class="logo-text"
        ...
        <VuePerfectScrollbar ref="mainSidebarPs" class="scroll-area--main-sidebar pt-2" :settings="settings" @ps-scroll-y="psSectionScroll">
          <template v-for="(sidebarItem, index) in sidebarItems">
            ...
            // :class="[{'router-link-active': activeLink}]" :target="target" exact
            -->

            <router-link :key="`header-${index}`" v-if="sidebarItem.header && !sidebarItemsMin" class="navigation-header truncate" :to="sidebarItem.url" exact>
...
export default {
  ...
  asyncComputed: {
    logoIcono: {
      async get() {
        return require('@/assets/images/pages/4-materialize.png')
        // if (this.$store.state.logoCompaniaIconoSeleccionada) {
        //   return await this.convertPath(this.$store.state.logoCompaniaIconoSeleccionada);
        // }
        // return null;
      }
    },
    logo: {
      async get() {
        return require('@/assets/images/leo-fintech/progrexa_logoB.png')
        // if (this.$store.state.logoCompaniaSeleccionada) {
        //   return await this.convertPath(this.$store.state.logoCompaniaSeleccionada);
        // }
        // return null;
      }
    }
  ...
  methods: {
    ...
    setSidebarWidth() {
      this.$store.commit('UPDATE_WINDOW_WIDTH', this.windowWidth);

      if (this.windowWidth < 1200) {
        if (this.windowWidth < 576) {
          this.$store.commit('UPDATE_WINDOW_BREAKPOINT', 'sm');
        ...
        if (this.reduceSidebar) this.$store.dispatch('updateSidebarWidth', 'reduced');
